        ...
        */
        ...
        M.getPlantDesc = function (me) {
            ...
            if (me.children.length > 0) {
                ...
                for (let i in me.children) {
                    if (!M.plants[me.children[i]]) console.log('No plant named ' + me.children[i]);
                        ...
                        if (it.unlocked)
                            children += '<div class="gardenSeedTiny" style="background-position:' + -0 * 48 + 'px ' + -it.icon * 48 + 'px;"></div>';
                        else
                            children +=
            ...
            return (
                ...
                Game.sayTime((100 / (me.ageTick + me.ageTickR / 2)) * (me.mature / 100) * M.stepT * 30, -1) +
                ' <small>(' +
                ...
                let str =
                    ...
                    (M.parent.amount < me.req
                        ...
                        (M.soil == me.id
                            ...
                            : M.nextSoil > Date.now()
                                ? 'You will be able to change your soil again in ' + Game.sayTime(((M.nextSoil - Date.now()) / 1000) * 30 + 30, -1) + '.'
            ...
            return function () {
                ...
                if (tile[0] == 0) {
                    ...
                    let str =
                        ...
                        (stage < 4
                            ? 'Mature in about ' +
                            Game.sayTime((100 / (M.plotBoost[y][x][0] * (me.ageTick + me.ageTickR / 2))) * ((me.mature - tile[1]) / 100) * M.stepT * 30, -1) +
                            ' (' +
                            Beautify(Math.ceil((100 / (M.plotBoost[y][x][0] * (me.ageTick + me.ageTickR / 2))) * ((me.mature - tile[1]) / 100))) +
                            ' tick' +
                            (Math.ceil((100 / (M.plotBoost[y][x][0] * (me.ageTick + me.ageTickR / 2))) * ((me.mature - tile[1]) / 100)) == 1 ? '' : 's') +
                            ')'
                            : !me.immortal
                                ? 'Decays in about ' +
                                Game.sayTime((100 / (M.plotBoost[y][x][0] * (me.ageTick + me.ageTickR / 2))) * ((100 - tile[1]) / 100) * M.stepT * 30, -1) +
                                ' (' +
                                Beautify(Math.ceil((100 / (M.plotBoost[y][x][0] * (me.ageTick + me.ageTickR / 2))) * ((100 - tile[1]) / 100))) +
                                ' tick' +
                                (Math.ceil((100 / (M.plotBoost[y][x][0] * (me.ageTick + me.ageTickR / 2))) * ((100 - tile[1]) / 100)) == 1 ? '' : 's') +
                                ')'
                                : 'Does not decay') +
                        '</small>' +
        ...
        M.buildPanel = function () {
            ...
            for (let i in M.plants) {
                ...
                str +=
                    ...
                    ' locked" ' +
                    Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.seedTooltip(' + me.id + ')', 'this') +
                    '>';
                str +=
            ...
            for (let i in M.tools) {
                ...
                str +=
                    ...
                    '" ' +
                    Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.toolTooltip(' + me.id + ')', 'this') +
                    '>';
                str +=
            ...
            for (let i in M.soils) {
                ...
                str +=
                    ...
                    (M.soil == me.id ? ' on' : '') +
                    '" ' +
                    Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.soilTooltip(' + me.id + ')', 'this') +
                    '>';
                str +=
            ...
            for (let i in M.soils) {
                ...
                $('gardenSoil-' + me.id, true).addEventListener(
                    ...
                    (function (me) {
                        return function () {
                            if (M.freeze || M.soil == me.id || M.nextSoil > Date.now() || M.parent.amount < me.req) {
                                return false;
                            }
                            PlaySound('snd/toneTick.mp3');
                ...
                for (let y = 0; y < 6; y++) {
                    for (let x = 0; x < 6; x++) {
                        str +=
                            ...
                            y * M.tileSize +
                            'px;display:none;" ' +
                            Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.tileTooltip(' + x + ',' + y + ')', 'this') +
                            '>';
        ...
        M.askConvert = function () {
            ...
            Game.Prompt(
                ...
                [['Yes!', 'Game.ClosePrompt();Game.ObjectsById[' + M.parent.id + '].minigame.convert();'], 'No']
            );
        };
        M.convert = function () {
            if (M.plantsUnlockedN < M.plantsN) return false;
                                    ...
                                    if (any > 0) {
                                        ...
                                        for (let ii = 0; ii < muts.length; ii++) {
                                            if (
                                                ...
                                                ((!M.plants[muts[ii][0]].weed && !M.plants[muts[ii][0]].fungus) || Math.random() < M.plotBoost[y][x][2])
                                            )
                                                list.push(muts[ii][0]);
                                        }
