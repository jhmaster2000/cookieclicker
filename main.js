...
const ccfetch = async function (
    /** @type {string} */ url,
    /** @type {(arg0: string) => void} */ callback
) {
	const urlObj = new URL(url, window.location.origin);
	urlObj.searchParams.set('nocache', Date.now().toString());
	const res = await fetch(urlObj, { method: 'GET', headers: { 'Content-Type': 'text/plain' } });
...
function BeautifyAll() { //run through upgrades and achievements to reformat the numbers
	...
	for (let i in Game.UpgradesById) { Game.UpgradesById[i].ddesc = BeautifyInText(Game.UpgradesById[i].ddesc); }
	for (let i in Game.AchievementsById) { Game.AchievementsById[i].ddesc = BeautifyInText(Game.AchievementsById[i].ddesc); }
}
...
function pack2(/* string */ values) {
	let chunks = values.match(/.{1,14}/g), packed = '';
...
// @ts-expect-error
let saveAs = function (view) { "use strict"; if (typeof navigator !== "undefined" && /MSIE [1-9]\./.test(navigator.userAgent)) { return; } let doc = view.document, get_URL = function () { return view.URL || view.webkitURL || view; }, save_link = doc.createElementNS("http://www.w3.org/1999/xhtml", "a"), can_use_save_link = "download" in save_link, click = function (node) { let event = new MouseEvent("click"); node.dispatchEvent(event); }, is_safari = /Version\/[\d\.]+.*Safari/.test(navigator.userAgent), webkit_req_fs = view.webkitRequestFileSystem, req_fs = view.requestFileSystem || webkit_req_fs || view.mozRequestFileSystem, throw_outside = function (ex) { (view.setImmediate || view.setTimeout)(function () { throw ex; }, 0); }, force_saveable_type = "application/octet-stream", fs_min_size = 0, arbitrary_revoke_timeout = 500, revoke = function (file) { let revoker = function () { if (typeof file === "string") { get_URL().revokeObjectURL(file); } else { file.remove(); } }; if (view.chrome) { revoker(); } else { setTimeout(revoker, arbitrary_revoke_timeout); } }, dispatch = function (filesaver, event_types, event) { event_types = [].concat(event_types); let i = event_types.length; while (i--) { let listener = filesaver["on" + event_types[i]]; if (typeof listener === "function") { try { listener.call(filesaver, event || filesaver); } catch (ex) { throw_outside(ex); } } } }, auto_bom = function (blob) { if (/^\s*(?:text\/\S*|application\/xml|\S*\/\S*\+xml)\s*;.*charset\s*=\s*utf-8/i.test(blob.type)) { return new Blob(["\ufeff", blob], { type: blob.type }); } return blob; }, FileSaver = function (blob, name, no_auto_bom) { if (!no_auto_bom) { blob = auto_bom(blob); } let filesaver = this, type = blob.type, blob_changed = false, object_url, target_view, dispatch_all = function () { dispatch(filesaver, "writestart progress write writeend".split(" ")); }, fs_error = function () { if (target_view && is_safari && typeof FileReader !== "undefined") { let reader = new FileReader; reader.onloadend = function () { let base64Data = reader.result; target_view.location.href = "data:attachment/file" + base64Data.slice(base64Data.search(/[,;]/)); filesaver.readyState = filesaver.DONE; dispatch_all(); }; reader.readAsDataURL(blob); filesaver.readyState = filesaver.INIT; return; } if (blob_changed || !object_url) { object_url = get_URL().createObjectURL(blob); } if (target_view) { target_view.location.href = object_url; } else { let new_tab = view.open(object_url, "_blank"); if (new_tab == undefined && is_safari) { view.location.href = object_url; } } filesaver.readyState = filesaver.DONE; dispatch_all(); revoke(object_url); }, abortable = function (func) { return function () { if (filesaver.readyState !== filesaver.DONE) { return func.apply(this, arguments); } }; }, create_if_not_found = { create: true, exclusive: false }, slice; filesaver.readyState = filesaver.INIT; if (!name) { name = "download"; } if (can_use_save_link) { object_url = get_URL().createObjectURL(blob); setTimeout(function () { save_link.href = object_url; save_link.download = name; click(save_link); dispatch_all(); revoke(object_url); filesaver.readyState = filesaver.DONE; }); return; } if (view.chrome && type && type !== force_saveable_type) { slice = blob.slice || blob.webkitSlice; blob = slice.call(blob, 0, blob.size, force_saveable_type); blob_changed = true; } if (webkit_req_fs && name !== "download") { name += ".download"; } if (type === force_saveable_type || webkit_req_fs) { target_view = view; } if (!req_fs) { fs_error(); return; } fs_min_size += blob.size; req_fs(view.TEMPORARY, fs_min_size, abortable(function (fs) { fs.root.getDirectory("saved", create_if_not_found, abortable(function (dir) { let save = function () { dir.getFile(name, create_if_not_found, abortable(function (file) { file.createWriter(abortable(function (writer) { writer.onwriteend = function (event) { target_view.location.href = file.toURL(); filesaver.readyState = filesaver.DONE; dispatch(filesaver, "writeend", event); revoke(file); }; writer.onerror = function () { let error = writer.error; if (error.code !== error.ABORT_ERR) { fs_error(); } }; "writestart progress write abort".split(" ").forEach(function (event) { writer["on" + event] = filesaver["on" + event]; }); writer.write(blob); filesaver.abort = function () { writer.abort(); filesaver.readyState = filesaver.DONE; }; filesaver.readyState = filesaver.WRITING; }), fs_error); }), fs_error); }; dir.getFile(name, { create: false }, abortable(function (file) { file.remove(); save(); }), abortable(function (ex) { if (ex.code === ex.NOT_FOUND_ERR) { save(); } else { fs_error(); } })); }), fs_error); }), fs_error); }, FS_proto = FileSaver.prototype, saveAs = function (blob, name, no_auto_bom) { return new FileSaver(blob, name, no_auto_bom); }; if (typeof navigator !== "undefined" && navigator.msSaveOrOpenBlob) { return function (blob, name, no_auto_bom) { if (!no_auto_bom) { blob = auto_bom(blob); } return navigator.msSaveOrOpenBlob(blob, name || "download"); }; } FS_proto.abort = function () { let filesaver = this; filesaver.readyState = filesaver.DONE; dispatch(filesaver, "abort"); }; FS_proto.readyState = FS_proto.INIT = 0; FS_proto.WRITING = 1; FS_proto.DONE = 2; FS_proto.error = FS_proto.onwritestart = FS_proto.onprogress = FS_proto.onwrite = FS_proto.onabort = FS_proto.onerror = FS_proto.onwriteend = null; return saveAs; }(typeof self !== "undefined" && self || typeof window !== "undefined" && window || this.content); if (typeof module !== "undefined" && module.exports) { module.exports.saveAs = saveAs; } else if (typeof define !== "undefined" && define !== null && define.amd != null) { define([], function () { return saveAs; }); }
...
function AddEvent(el, ev, func) {
	...
	else if (el.attachEvent) { let func2 = function () { func.call(el); }; el.attachEvent('on' + ev, func2); return [el, ev, func2]; }
	return false;
}
...
Game.Launch = function () {
	...
	if (day >= 41 && day <= 46) Game.baseSeason = 'valentines';
		...
		let easterDay = function (Y) { let C = Math.floor(Y / 100); let N = Y - 19 * Math.floor(Y / 19); let K = Math.floor((C - 17) / 25); let I = C - Math.floor(C / 4) - Math.floor((C - K) / 3) + 19 * N + 15; I = I - 30 * Math.floor((I / 30)); I = I - Math.floor(I / 28) * (1 - Math.floor(I / 28) * Math.floor(29 / (I + 1)) * Math.floor((21 - N) / 11)); let J = Y + Math.floor(Y / 4) + I + 2 - C + Math.floor(C / 4); J = J - 7 * Math.floor(J / 7); let L = I - J; let M = 3 + Math.floor((L + 40) / 44); let D = L + 28 - 31 * Math.floor(M / 4); return new Date(Y, M - 1, D); }(year);
		// @ts-expect-error
	...
	Game.updateLog =
		`<div class="selectable"><div class="section">${loc("Info")}</div><div class="subsection"><div class="title">${loc("About")}</div><div class="listing">${loc("Cookie Clicker is a javascript game by %1 and %2.", ['<a href="//orteil.dashnet.org" target="_blank">Orteil</a>', '<a href="//dashnet.org" target="_blank">Opti</a>'])}</div><div class="listing">${EN ?
			...
			loc("Useful links: %1, %2, %3, %4.", [
				...
				[`${EN ? 'get ' : ''}<a href="https://store.steampowered.com/app/1454400/Cookie_Clicker/" target="_blank" class="highlightHover smallWhiteButton">Cookie Clicker on Steam</a>${EN ? ' (it\'s about 5 bucks)' : ''}<br><br>&bull; ${EN ? 'support us on ' : ''}<a href="https://www.patreon.com/dashnet" target="_blank" class="highlightHover smallOrangeButton">Patreon</a>${EN ? ' (there\'s perks!)' : ''}<br><br>&bull; ${EN ? 'check out our ' : ''}<a href="http://www.redbubble.com/people/dashnet" target="_blank" class="highlightHover smallWhiteButton">Shop</a>${EN ? ' with rad cookie shirts, hoodies and stickers' : ''}${(EN) ? '<br><br>&bull; disable your adblocker (if you want!)' : ''}`]
			)}</div></div><div class="listing warning">${loc("Note: if you find a new bug after an update and you're using a 3rd-party add-on, make sure it's not just your add-on causing it!")}</div>${(`<div class="listing warning">${loc("Warning: clearing your browser cache or cookies <small>(what else?)</small> will result in your save being wiped. Export your save and back it up first!")}</div>`)}</div><div class="subsection"><div class="title">${loc("Version history")}</div>`;

	for (let i = 0; i < locPatches.length; i++) {
	...
	Game.Init = function () {
		...
		Game.attachTooltip(l('heralds'), function () {
			...
			if (!Game.externalDataLoaded) str += loc("Heralds couldn't be loaded. There may be an issue with our servers, or you are playing the game locally.");
			else {
				if (Game.heralds === 0) str += loc("There are no heralds at the moment. Please consider <b style=\"color:#bc3aff;\">donating to our Patreon</b>!");
				else {
			...
			str += '<div class="line"></div><span style="font-size:90%;opacity:0.6;">' + (loc("<b>Heralds</b> are people who have donated to our highest Patreon tier, and are limited to 100.<br>Each herald gives everyone +1% CpS.<br>Heralds benefit everyone playing the game, regardless of whether you donated.")) + '</span><div class="line"></div>' + tinyIcon([21, 29]);
		...
		Game.WriteSave = function (type) {
			...
			for (let i in Game.Objects)//buildings
			{
				let me = Game.Objects[i];
				if (type == 3) str += '\n	' + me.name + ' : ';
				if (me.vanilla) {
			...
			let toCompress = [];
			for (let i in Game.UpgradesById)//upgrades
			{
			...
			toCompress = [];
			for (let i in Game.AchievementsById) { //achievements
			...
			if (str != '') {
				...
				if (str == '') return false;
					...
					if (version >= 1) {
						...
						let wrinklers = { amount: spl[23] ? parseFloat(spl[23]) : 0, number: spl[24] ? parseInt(spl[24]) : 0 };
						...
						for (let i in Game.vault) { Game.vault[i] = parseInt(Game.vault[i]); }
						...
						if (Game.Has('Season switcher')) { for (let i in Game.seasons) { Game.Unlock(Game.seasons[i].trigger); } }
		...
		Game.Reset = function (hard) {
			...
			if (!hard) {
				if (Game.ascensionMode != 1) {
					for (let i in Game.permanentUpgrades) {
		...
		Game.BuildAscendTree = function (justBought) {
			...
			for (let i in Game.PrestigeUpgrades) {
				...
				for (let ii in me.parents)//create pulsing links
				{
					...
					AddEvent(l('heavenlyUpgrade' + me.id), 'mousedown', function (me) {
						return function () {
							...
							if (Game.keys[16] && typeof LASTHEAVENLYSELECTED !== 'undefined' && me != LASTHEAVENLYSELECTED) {
								...
								if (parent) {
									// @ts-expect-error
									let origX = parent.posX; let origY = parent.posY;
									// @ts-expect-error
									let targX = LASTHEAVENLYSELECTED.posX; let targY = LASTHEAVENLYSELECTED.posY;
		...
		Game.mouseCps = function () {
			...
			let num = 0;
			for (let i in Game.Objects) { num += Game.Objects[i].amount; }
		...
		Game.CalculateGains = function () {
			...
			if (Game.Has('Golden switch [off]')) {
				...
				if (Game.Has('Residual luck')) {
					...
					for (let i in upgrades) { if (Game.Has(upgrades[i])) goldenSwitchMult += 0.1; }
				}
				...
				if (me.spawnsOnTimer && me.spawnConditions())//only run on shimmer types that work on a timer
				{
					if (!me.spawned)//no shimmer spawned for this type? check the timer and try to spawn one
					{
						...
						if (Math.random() < Math.pow(Math.max(0, (me.time - me.minTime) / (me.maxTime - me.minTime)), 5)) {
							...
							if (Game.Has('Distilled essence of redoubled luck') && Math.random() < 0.01) newShimmer = new Game.shimmer(i);
							me.spawned = 1;
						}
					}
				}
		...
		Game.textParticlesAdd = function (text, el, posX, posY) {
			...
			// @ts-expect-error
			for (let ii in Game.textParticles) { if (ii != i) (Game.textParticles[ii].l || l('particle' + ii)).style.zIndex = 100000000; }
		...
		Game.UpdateMenu = function () {
			...
			if (Game.onMenu == 'prefs') {
				...
				str +=
					...
					((`<div class="listing"><a class="option smallFancyButton" ${Game.clickStr}="Game.FileSave();PlaySound(\'snd/tick.mp3\');">${loc("Save to file")}</a><a class="option smallFancyButton" style="position:relative;"><input id="FileLoadInput" type="file" style="cursor:pointer;opacity:0;position:absolute;left:0px;top:0px;width:100%;height:100%;" onchange="Game.FileLoad(event);" ${Game.clickStr}="PlaySound(\'snd/tick.mp3\');"/>${loc("Load from file")}</a><label>${loc("Use this to keep backups on your computer")}</label></div>`)) +
					...
					// @ts-expect-error
					(Game.WritePrefButton('warn', 'warnButton', loc("Closing warning") + ON, loc("Closing warning") + OFF) + '<label>(' + loc("the game will ask you to confirm when you close the window") + ')</label><br>') +
					...
					// @ts-expect-error
					(Game.WritePrefButton('customGrandmas', 'customGrandmasButton', loc("Custom grandmas") + ON, loc("Custom grandmas") + OFF) + '<label>(' + loc("some grandmas will be named after Patreon supporters") + ')</label><br>') +
					// @ts-expect-error
					...
					(`<div class="listing"><a class="option smallFancyButton" ${Game.clickStr}="Game.CheckModData();PlaySound(\'snd/tick.mp3\');">${loc("Check mod data")}</a><label>(${loc("view and delete save data created by mods")})</label></div>`) +
				...
				let list = [];
				//sort the upgrades
				for (let i in Game.Upgrades) { list.push(Game.Upgrades[i]); }//clone first
				let sortMap = function (a, b) {
		...
		Game.Object = function (name, commonName, desc, icon, iconColumn, art, price, cps, buyFunction) {
			...
			this.sell = function (amount, bypass) {
				...
				if (success && Game.shimmerTypes['golden'].n <= 0 && Game.auraMult('Dragon Orbs') > 0) {
					...
					for (let i in Game.Objects) { if (Game.Objects[i].amount > 0) highestBuilding = Game.Objects[i]; }
					// @ts-expect-error
				...
				if (me.amount > 0) {
					...
					for (let i in me.synergies) {
						...
						if (Game.Has(it.name)) {
							...
							let boost = (other.storedTotalCps * Game.globalCpsMult) - (other.storedTotalCps * Game.globalCpsMult) / (1 + me.amount * weight);
							synergyBoost += boost;
							if (!synergiesWith[other.plural]) synergiesWith[other.plural] = 0;
			...
			this.levelUp = function (me) {
				return function (free) {
					Game.spendLump(me.level + 1, loc("level up your %1", me.plural), function () {
						...
						me.refresh();
						// @ts-expect-error
						if (l('productLevel' + me.id)) { let rect = l('productLevel' + me.id).getBounds(); Game.SparkleAt((rect.left + rect.right) / 2, (rect.top + rect.bottom) / 2 - 24 + 32 - TopBarOffset); }
						// @ts-expect-error
		...
		new Game.Object('Cursor', 'cursor|cursors|clicked|[X] extra finger|[X] extra fingers', 'Autoclicks once every 10 seconds.', 0, 0, {}, 15, function (me) {
			...
			let mult = 1;
			let num = 0;
			for (let i in Game.Objects) { if (Game.Objects[i].name != 'Cursor') num += Game.Objects[i].amount; }
			add = add * num;
		...
		new Game.Object('Grandma', 'grandma|grandmas|baked|Grandmas are [X] year older|Grandmas are [X] years older', 'A nice grandma to bake more cookies.', 1, 1, {
			...
			let num = 0;
			for (let i in Game.Objects) { if (Game.Objects[i].name != 'Grandma') num += Game.Objects[i].amount; }
		...
		for (let i in Game.Objects) {
			...
			if (me.id > 0) {
				...
				AddEvent(me.canvas, 'mousemove', function (me) { return function (e) { let box = this.getBounds(); me.mousePos[0] = e.pageX - box.left; me.mousePos[1] = e.pageY - box.top; }; }(me));
			}
		}
				...
				if (this.choicesFunction) {
					if (Game.choiceSelectorOn == this.id) {
						...
						if (typeof choices === 'string') {
							...
							let selected = 0;
							// @ts-expect-error
							for (let i in choices) { if (choices[i].selected) selected = i; }
		...
		let getStrCookieProductionMultiplierPlus = function (x) { return strCookieProductionMultiplierPlus.replace('[x]', x); };
		let getStrThousandFingersGain = function (x) { return loc("Multiplies the gain from %1 by <b>%2</b>.", [getUpgradeName("Thousand fingers"), x]); };
		let strKittenDesc = loc("You gain <b>more CpS</b> the more milk you have.");
		let getStrClickingGains = function (x) { return loc("Clicking gains <b>+%1% of your CpS</b>.", x); };
		...
		for (let i in Game.Tiers) { Game.Tiers[i].upgrades = []; }
		...
		Game.UnlockTiered = function (me) {
			for (let i in me.tieredUpgrades) { if (Game.Tiers[me.tieredUpgrades[i].tier].unlock != -1 && me.amount >= Game.Tiers[me.tieredUpgrades[i].tier].unlock) Game.Unlock(me.tieredUpgrades[i].name); }
			for (let i in me.tieredAchievs) { if (me.amount >= Game.Tiers[me.tieredAchievs[i].tier].achievUnlock) Game.Win(me.tieredAchievs[i].name); }
			for (let i in me.synergies) { let syn = me.synergies[i]; if (Game.Has(Game.Tiers[syn.tier].req) && syn.buildingTie1.amount >= Game.Tiers[syn.tier].unlock && syn.buildingTie2.amount >= Game.Tiers[syn.tier].unlock) Game.Unlock(syn.name); }
		};
		...
		Game.GrandmaSynergy = function (name, desc, building) {
			...
			let upgrade = new Game.Upgrade(name, desc, building.basePrice * Game.Tiers[2].price, [10, 9], function () { Game.Objects['Grandma'].redraw(); });
		...
		new Game.Upgrade('Season switcher', loc("Allows you to <b>trigger seasonal events</b> at will, for a price.") + '<q>There will always be time.</q>', 1111, [16, 6], function () { for (let i in Game.seasons) { Game.Unlock(Game.seasons[i].trigger); } }); Game.last.pool = 'prestige'; Game.last.parents = ['Heralds'];
		...
		order = 40000;
		new Game.Upgrade('Eternal seasons', loc("Seasons now last forever.") + '<q>Season to taste.</q>', 7, [16, 6], function () { for (let i in Game.seasons) { Game.Unlock(Game.seasons[i].trigger); } });//debug purposes only
		Game.last.pool = 'debug';
		...
		for (let i in Game.eggDrops)//scale egg prices to how many eggs you have
		{ Game.Upgrades[Game.eggDrops[i]].priceFunc = function () { return Math.pow(2, Game.GetHowManyEggs()) * 999; }; }

		for (let i in Game.rareEggDrops) { Game.Upgrades[Game.rareEggDrops[i]].priceFunc = function () { return Math.pow(3, Game.GetHowManyEggs()) * 999; }; }
		...
		for (let i in gardenDrops)//scale by CpS
		{
		...
		let getCookiePrice = function (level) { return 999999999999999999999999999999999999999 * Math.pow(10, (level - 1) / 2); };
		...
		Game.NewUnshackleUpgradeTier = function (obj) {
			...
			let upgrade = new Game.Upgrade(obj.tier == 1 ? 'Unshackled flavor' : 'Unshackled ' + tier.name.toLowerCase(), loc("Unshackles all <b>%1-tier upgrades</b>, making them more powerful.<br>Only applies to unshackled buildings.", cap(loc("[Tier]" + tier.name, 0, tier.name))) + (EN ? '<q>' + obj.q + '</q>' : ''), Math.pow(obj.tier, 7.5) * 10000000, [10, tier.iconRow]);
			upgrade.pool = 'prestige';
		...
		// @ts-expect-error
		if (Music) { for (let i in Music.tracks) { Game.jukebox.tracks.push(Music.tracks[i].name); } }
		...
		Game.NewUpgradeCookie({ name: 'Steamed cookies', desc: 'Localized entirely within this gaming platform? Yes! Baked with the power of steam, in a touch of cutting-edge modernity not seen since the industrial revolution.' + ('<br>Only of any use in Cookie Clicker\'s Steam version, of course.'), icon: [26, 35], power: 0, price: getCookiePrice(52) }); Game.last.pool = 'debug';
		...
		for (let i in Game.UnlockAt) { Game.Upgrades[Game.UnlockAt[i].name].unlockAt = Game.UnlockAt[i]; }
		for (let i in Game.Upgrades) { if (Game.Upgrades[i].pool == 'prestige') Game.Upgrades[i].order = Game.Upgrades[i].id; }
		...
		for (let i in Game.UpgradePositions) { Game.UpgradesById[i].posX = Game.UpgradePositions[i][0]; Game.UpgradesById[i].posY = Game.UpgradePositions[i][1]; }
		...
		order = 61000;
		let achiev;
		achiev = new Game.Achievement('Getting even with the oven', EN ? 'Defeat the <b>Sentient Furnace</b> in the factory dungeons.' : '???', [12, 7]); Game.last.pool = 'dungeon';
		achiev = new Game.Achievement('Now this is pod-smashing', EN ? 'Defeat the <b>Ascended Baking Pod</b> in the factory dungeons.' : '???', [12, 7]); Game.last.pool = 'dungeon';
		achiev = new Game.Achievement('Chirped out', EN ? 'Find and defeat <b>Chirpy</b>, the dysfunctionning alarm bot.' : '???', [13, 7]); Game.last.pool = 'dungeon';
		achiev = new Game.Achievement('Follow the white rabbit', EN ? 'Find and defeat the elusive <b>sugar bunny</b>.' : '???', [14, 7]); Game.last.pool = 'dungeon';
		...
		*/
							...
							if (Game.keys[17] && Game.sesame) { me.type = !me.type; PlaySound('snd/shimmerClick.mp3'); }//ctrl-click on a wrinkler in god mode to toggle its shininess
								...
								if (Game.prefs.particles && !Game.prefs.notScary && !Game.WINKLERS && !(me.hp <= 0.5 && me.phase > 0)) {
									...
									for (let ii = 0; ii < 3; ii++) {
										...
										let part = Game.particleAdd(x, y, Math.random() * 4 - 2, Math.random() * -2 - 2, 1, 1, 2, me.type == 1 ? 'shinyWrinklerBits.png' : 'wrinklerBits.png');
										part.r = -me.r;
									}
								}
							}
		...
		if (!EN) { for (let i in Game.santaLevels) { Game.santaLevels[i] = loc(Game.santaLevels[i]); } }
		for (let i in Game.santaDrops)//scale christmas upgrade prices with santa level
		...
		Game.dragonLevels = [
			...
			{ name: 'Krumblor, cookie dragon', action: 'Train Supreme Intellect<br><small>Aura : confers various powers to your minigames</small>', pic: 5, },
			{
				...
				cost: function () { let fail = 0; for (let i in Game.Objects) { if (Game.Objects[i].amount < 50) fail = 1; } return (fail == 0); },
				buy: function () { for (let i in Game.Objects) { Game.Objects[i].sacrifice(50); } Game.Unlock('Dragon cookie'); },
			...
			},
			{
				...
				cost: function () { let fail = 0; for (let i in Game.Objects) { if (Game.Objects[i].amount < 200) fail = 1; } return (fail == 0); },
				buy: function () { for (let i in Game.Objects) { Game.Objects[i].sacrifice(200); } },
		...
		Game.dragonAurasBN = {}; for (let i in Game.dragonAuras) { Game.dragonAurasBN[Game.dragonAuras[i].name] = Game.dragonAuras[i]; }
		for (let i in Game.dragonAuras) { Game.dragonAuras[i].id = parseInt(i); Game.dragonAuras[i].dname = loc(Game.dragonAuras[i].name); }
			...
			if (Game.OnAscend) {
				...
				if (Game.AscendTimer == 0) {
					if (Game.prefs.particles) {
						...
						{
							ctx.globalAlpha = 1;
							let amount = Math.floor(Game.cookies).toString();
		...
		Game.OpenSesame = function () {
			...
			str += '<a class="option neato" ' + Game.clickStr + '="for (let i in Game.Objects){Game.Objects[i].level=0;Game.Objects[i].onMinigame=false;Game.Objects[i].refresh();}Game.recalculateGains=1;">Reset levels</a>';
			...
			for (let i = 0; i < Game.goldenCookieChoices.length / 2; i++) {
				str += '<a class="option neato" ' + Game.clickStr + '="let newShimmer=new Game.shimmer(\'golden\');newShimmer.force=\'' + Game.goldenCookieChoices[i * 2 + 1] + '\';">' + Game.goldenCookieChoices[i * 2] + '</a>';
	...
	Game.Logic = function () {
		...
		if (!Game.toSave && !Game.isSaving) {
			if (Game.toReload) { Game.toReload = false; location.reload(); }
			if (Game.toQuit) { Game.toQuit = false; window.close(); }
		}
